...
- `/catalog` and the approved communication-course detail route now render. `/paths`, `/partners`, `/business/dashboard`, `/login`, `/register`, `/about`, `/contact`, and other linked destinations remain unimplemented and may return 404. The company dashboard remains behind the rejected-visual Human Approval gate; other pages require their design sequence before implementation.
...
- 2026-09-25 catalog/detail implementation: `/catalog` and `/catalog/communication-skills` compile as statically rendered routes. `npm test` (17/17), `npx tsc --noEmit`, `npm run build`, and `git diff --check` pass. Search and category query states render server-side. This does not validate real course data or commerce.
