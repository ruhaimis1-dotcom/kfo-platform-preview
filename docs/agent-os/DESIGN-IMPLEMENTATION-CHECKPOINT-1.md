...
- Revised the hero composition to match the approved full-page checkpoint reference: KFO wordmark/navigation, people imagery on the left, headline and actions on the right, with the same dark forest and signal green treatment. The photo itself is now under owner review as described below.
...
- Following owner feedback that the approved learner-pair hero photo was not convincing, added a new generated training-workshop photo as a **preview candidate**. It depicts a trainer and learner and keeps the existing hero layout, copy, palette, and brand styling. This image is not part of the approved checkpoint and remains unapproved; the previously used checkpoint crop is retained for easy rollback.
- Verified on the deployed preview that IBM Plex Sans Arabic is loaded and active for page text, and Inter is loaded. Applied the approved Inter-first stack to mixed Latin/numeric data fields; Arabic glyphs fall back to IBM Plex Sans Arabic.
...
- Original vector logo artwork and reusable source photography are not present. The current hero photo is a generated, unapproved review candidate; approved checkpoint crops remain in the repository for rollback. Other preview images are crops from approved checkpoint renders. None replaces trusted production originals.
...
- Public-home revision on 2026-09-25 renders on the branch Preview. Desktop browser QA confirms the approved hero layout and the workshop-image candidate loads; its visual approval is pending. `npm test` (17/17), `npx tsc --noEmit`, `npm run build`, and `git diff --check` pass for this revision.
...
- 2026-09-25 font/hero revision: browser confirms IBM Plex Sans Arabic and Inter load in Preview; Inter is explicitly applied before Arabic fallback on mixed numeric fields. The new workshop hero image is for owner visual review only; approval is still pending.
