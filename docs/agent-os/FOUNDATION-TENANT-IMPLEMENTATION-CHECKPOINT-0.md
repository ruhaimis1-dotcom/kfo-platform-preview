...
- API/service isolation tests and runtime Storage API list/read/write/delete/signed-access tests remain. Supabase blocks direct SQL deletion from `storage.objects`; validate deletion through the Storage API harness. No Next.js application shell exists yet, so trusted-host/Auth integration and RTL shell QA remain.
