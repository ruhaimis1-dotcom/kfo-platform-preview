...
- RLS policies must be treated as required security code; a table grant without RLS is a release blocker. Service-role credentials bypass RLS and must remain server-only, narrowly used and audited.
